...
void glCamera::SetPerspective()
{
  ...
  //x = sin(RAD(m_HeadingDegrees))*m_Test.i + cos(RAD(m_HeadingDegrees))*m_Test.j;
  //y = cos(RAD(m_HeadingDegrees))*m_Test.i + sin(RAD(m_HeadingDegrees))*m_Test.j;

  m_Position.x += m_Test.i;
  m_Position.y += m_Test.j;
  m_Position.z += m_Test.k;

	// Translate to our new position.
	glTranslatef(-m_Position.x, -m_Position.y, m_Position.z);

  TRACE("Pitch = %.1f  Heading = %.1f\n", m_PitchDegrees, m_HeadingDegrees);
  TRACE("xa = %.1f ya = %.1f za = %.1f\n", x, y, z);
...
void glCamera::SetDirectionVector(float x, float y, float z)
{
  m_Test.i = x;
  m_Test.j = y;
  m_Test.k = z;
...
void glCamera::SetVelocity(float vel)
{
  m_ForwardVelocity = vel;
}
